    ...
    for (int k=1; k<=K; k++) { //iterate through all clusters
        ...
        if (inds_k.count()>0) { //if the cluster is non-empty
            ...
            #pragma omp critical
            {
                ...
                clips_k=get_subclips(clips,inds_k); //retrieve the clips corresponding to this cluster
            }
            Define_Shells_Opts opts2; opts2.min_shell_size=opts.min_shell_size; opts2.shell_increment=opts.shell_increment;
            QList<Subcluster> subclusters0=compute_subcluster_detectability_scores(raw,clips_k,channel,opts2); //actually compute the scores
            #pragma omp critical
            {
...
QList<Shell> define_shells(const QList<double> &peaks,const Define_Shells_Opts &opts) {
    ...
    if (compute_min(peaks)<0) {
        ...
        for (int i=shells_neg.count()-1; i>=0; i--) {
            ...
            QList<long> new_inds; for (int i=0; i<SS.inds.count(); i++) new_inds << inds_neg[SS.inds[i]];
            SS.inds=new_inds;
            shells << SS;
        }
        for (int i=0; i<shells_pos.count(); i++) {
            Shell SS=shells_pos[i];
            QList<long> new_inds; for (int i=0; i<SS.inds.count(); i++) new_inds << inds_pos[SS.inds[i]];
            SS.inds=new_inds;
            shells << SS;
        }
        return shells;
...
QList<Subcluster> compute_subcluster_detectability_scores(DiskReadMda &pre,Mda &clips,int channel,const Define_Shells_Opts &opts) {
    QTime timer; timer.start();
    int M=clips.N1();
    int T=clips.N2();
    int L=clips.N3();
    ...
    Mda noise_shape;
    #pragma omp critical
    noise_shape=estimate_noise_shape(pre,T,channel); //estimate the noise shape corresponding to detection on this particular channel
