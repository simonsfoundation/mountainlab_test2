...
bool bandpass_filter_Processor::run(const QMap<QString, QVariant> &params)
{
	...
	double freq_wid = params.value("freq_wid", 1000).toDouble();  // defaults
    long chunkSize = params.value("processing_chunk_size", -1).toLongLong();
    long overlapSize = params.value("chunk_overlap_size", -1).toLongLong();
