...
void MVCdfView::paintEvent(QPaintEvent* evt)
{
    ...
    for (int i = 0; i < d->m_times.count(); i++) {
        ...
        if (time0 > max_time)
            max_time = time0;
        if (label_times[label0].count() > max_count)
            max_count = label_times[label0].count();
    }
    ...
    d->m_painter_paths.clear();
    d->m_painter_paths << QPainterPath();
