...
QString DiskArrayModel::path() { return d->m_path; }
...
bool do_minmax_downsample(QString path1, QString path2, long factor,
    QProgressDialog* dlg, bool first)
{
    FILE* inf = jfopen(path1.toLatin1().data(), "rb");
    if (!inf) {
        qWarning() << "Problem reading data from mda for do_minmax_downsample: "
                   << path1 << factor;
        return false;
    }
    FILE* outf = jfopen(path2.toLatin1().data(), "wb");
    if (!outf) {
        qWarning() << "Problem writing data to mda for do_minmax_downsample: "
                   << path2 << factor;
        jfclose(inf);
        return false;
    }
    ...
    if (data_type == MDAIO_TYPE_BYTE) {
        ...
        for (long kk = 0; kk < 2; kk++) { // determines min or max
            if (dim3 == 1) {
                fseek(inf, header_size,
                    SEEK_SET); // need to go back to first part in this case
            }
            for (long i = 0; i < dim2b; i++) {
                ...
                if (numread < factor * dim1) {
                    qWarning() << "Warning, unexpected problem in do_minmax_downsample"
                               << kk << i << numread << factor << dim1;
                }
...
void DiskArrayModel::createFileHierarchyIfNeeded()
{
    ...
    while (d->m_num_timepoints / scale0 > 1) {
        dlg.setLabelText(QString("Creating file hierarchy %1")
                             .arg(d->m_num_timepoints / scale0));
        qApp->processEvents();
        QString new_path = d->get_multiscale_file_name(scale0);
        if (!QDir(QFileInfo(new_path).path()).exists()) {
            // the following line is crazy!!
            QDir(QFileInfo(QFileInfo(new_path).path()).path())
                .mkdir(QFileInfo(QFileInfo(new_path).path()).fileName());
        }
        if (!do_minmax_downsample(last_path, new_path, MULTISCALE_FACTOR, &dlg,
                (scale0 == MULTISCALE_FACTOR))) {
            exit(-1);
            return;
        }
  ...
  while (d->m_num_timepoints/scale0>1) {
		  int N2=X.size(1)/MULTISCALE_FACTOR; if
  (N2*MULTISCALE_FACTOR<X.size(1)) N2++;
...
QString DiskArrayModelPrivate::get_code(long scale, long chunk_size,
    long chunk_ind)
{
...
Mda* DiskArrayModelPrivate::load_chunk(long scale, long chunk_ind)
{
    ...
    // printf("Loading chunk %d/%d\n",scale,chunk_ind); QTime timer;
    // timer.start();
    QString path = get_multiscale_file_name(scale);
    Mda* X = load_chunk_from_file(path, scale, chunk_ind);
    if (X) {
        // qDebug()  << "################################  Loaded chunk"  << scale
        // << chunk_ind << X->totalSize()*4*1.0/1000000;
        m_loaded_chunks[code] = X;
    }
    return X;
}

Mda* DiskArrayModelPrivate::load_chunk_from_file(QString path, long scale,
    long chunk_ind)
{
    if (scale == 1) {
        QVector<double> d0 = load_data_from_mda(path, m_num_channels * m_chunk_size,
            0, chunk_ind * m_chunk_size, 0);
        if (d0.isEmpty())
            return 0;
        Mda* ret = new Mda();
        ...
        QVector<double> d0 = load_data_from_mda(path, m_num_channels * m_chunk_size,
            0, chunk_ind * m_chunk_size, 0);
        QVector<double> d1 = load_data_from_mda(path, m_num_channels * m_chunk_size,
            0, chunk_ind * m_chunk_size, 1);
...
QVector<double> DiskArrayModelPrivate::load_data_from_mda(QString path, long n,
    long i1, long i2,
    long i3)
{
    QVector<double> ret;
    ...
    if (!inf) {
        qWarning() << "Problem reading data from mda: " << path << i1 << i2 << i3
                   << n;
        return ret;
    }
...
long DiskArrayModel::dim3() { return d->m_dim3; }
...
void DiskArrayModelPrivate::clean_up_file_hierarchies_in_path(
    const QString& path)
{
