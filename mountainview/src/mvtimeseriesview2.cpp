...
void MVTimeSeriesView2::setSampleRate(double samplerate)
{
...
void MVTimeSeriesView2::setTimesLabels(const QVector<double>& times, const QVector<int>& labels)
{
    d->m_times = times;
    d->m_labels = labels;
    update();
...
MVRange MVTimeSeriesView2::timeRange() const
{
...
void MVTimeSeriesView2::setTimeRange(MVRange range)
{
    if (range.min < 0) {
...
void MVTimeSeriesView2::setCurrentTimepoint(double t)
{
    d->m_view_agent->setCurrentTimepoint(t);
    update();
}

void MVTimeSeriesView2::setSelectedTimeRange(MVRange range)
{
...
void MVTimeSeriesView2::setActivated(bool val)
{
...
double MVTimeSeriesView2::currentTimepoint() const
{
    return d->m_view_agent->currentTimepoint();
}

void MVTimeSeriesView2::paintEvent(QPaintEvent* evt)
{
    Q_UNUSED(evt)
...
void MVTimeSeriesView2::mousePressEvent(QMouseEvent* evt)
{
...
void MVTimeSeriesView2::mouseReleaseEvent(QMouseEvent* evt)
{
    d->m_left_click_anchor_pix = QPointF(-1, -1);
    if (evt->button() == Qt::LeftButton) {
...
void MVTimeSeriesView2::mouseMoveEvent(QMouseEvent* evt)
{
    if (!d->m_left_click_dragging) {
...
void MVTimeSeriesView2::wheelEvent(QWheelEvent* evt)
{
    int delta = evt->delta();
...
void MVTimeSeriesView2::unit_test()
{
    ...
    */
    ...
    MVTimeSeriesView2* W = new MVTimeSeriesView2(new MVViewAgent); //note that the view agent does not get deleted. :(
    W->setTimeseries(X0);
...
void MVTimeSeriesView2::slot_scroll_to_current_timepoint()
{
    d->scroll_to_current_timepoint();
}

QList<mvtsv_channel> MVTimeSeriesView2Private::make_channel_layout(double W, double H, long M)
{
...
void MVTimeSeriesView2Private::paint_cursor(QPainter* painter, double W, double H)
{
    Q_UNUSED(W)
    Q_UNUSED(H)
...
void MVTimeSeriesView2Private::paint_markers(QPainter* painter, const QVector<double>& times, const QVector<int>& labels, double W, double H)
{
    Q_UNUSED(W)
    double mtop = m_prefs.mtop;
    double mbottom = m_prefs.mbottom;
...
void MVTimeSeriesView2Private::paint_message_at_top(QPainter* painter, QString msg, double W, double H)
{
    Q_UNUSED(H)
    QPen pen = painter->pen();
    pen.setColor(m_prefs.marker_color);
...
void MVTimeSeriesView2Private::paint_time_axis(QPainter* painter, double W, double H)
{
...
void MVTimeSeriesView2Private::paint_time_axis_unit(QPainter* painter, double W, double H, TickStruct TS)
{
    Q_UNUSED(W)
...
void MVTimeSeriesView2Private::paint_status_string(QPainter* painter, double W, double H, QString str)
{
    QPen pen = painter->pen();
    pen.setColor(Qt::black);
    painter->setPen(pen);
...
QPointF MVTimeSeriesView2Private::coord2pix(mvtsv_coord C)
{
...
mvtsv_coord MVTimeSeriesView2Private::pix2coord(long channel, QPointF pix)
{
...
void MVTimeSeriesView2Private::zoom_out(mvtsv_coord about_coord, double frac)
{
...
void MVTimeSeriesView2Private::zoom_in(mvtsv_coord about_coord, double frac)
{
    zoom_out(about_coord, 1 / frac);
}

void MVTimeSeriesView2Private::scroll_to_current_timepoint()
{
...
QString MVTimeSeriesView2Private::format_time(double tp)
{
    double samplerate = m_samplerate;
...
void MVTimeSeriesView2Private::update_cursor()
{
