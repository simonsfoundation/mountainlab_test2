...
bool DiskArrayModel::fileHierarchyExists() {
    ...
    return d->file_exists(d->get_multiscale_file_name());
}
...
void DiskArrayModel::createFileHierarchyIfNeeded() {
    ...
    DiskReadMda X(d->m_path);
    int M=X.N1();
    int N=X.N2();
    QList<long> multiscale_sizes;
    long size=1;
    while (size<N) {
        multiscale_sizes << size;
        size*=2;
    }
    if (multiscale_sizes.isEmpty()) multiscale_sizes << 1;
    long multiscale_N=0;
    foreach (long size,multiscale_sizes) multiscale_N+=size;
    long level=2;
    for (int i=0; i<num_multiscale_levels; i++) {
        multiscale_levels << level;
        level*=2;
    }
    DiskWriteMda Y; Y.open(MDAIO_TYPE_FLOAT32,output_path+".tmp",M,multiscale_N*2);

    long chunk_size=(long)pow(2,10);
    QList<double> chunk_mins,chunk_maxs;
    for (long timepoint=0; timepoint<N; timepoint+=chunk_size) {
        Mda chunk;
        X.readChunk(chunk,0,timepoint,M,chunk_size);

        long dsfactor=1;
        while (dsfactor<=chunk_size) {
