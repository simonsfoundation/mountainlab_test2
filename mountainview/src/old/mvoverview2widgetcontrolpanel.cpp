...
MVOverview2WidgetControlPanel::MVOverview2WidgetControlPanel(QWidget* parent)
    : QWidget(parent)
{
    ...
    { // Raw/Preprocessed Data
        ...
        d->add_combo_box(G, "timeseries_name", "Use data:");
...
QVariant MVOverview2WidgetControlPanel::getParameterValue(QString name, const QVariant& defaultval)
{
    ...
    if (d->m_checkbox_parameters.contains(name))
        return d->m_checkbox_parameters[name]->isChecked();
    if (d->m_combobox_parameters.contains(name))
        return d->m_combobox_parameters[name]->currentText();
...
{
    if (d->m_lineedit_parameters.contains(name))
        return d->m_lineedit_parameters[name]->setText(val.toString());
    if (d->m_checkbox_parameters.contains(name))
        return d->m_checkbox_parameters[name]->setChecked(val.toBool());
    if (d->m_combobox_parameters.contains(name))
        return d->m_combobox_parameters[name]->setCurrentText(val.toString());
    if (d->m_groupbox_parameters.contains(name)) {
        QGroupBox* G = d->m_groupbox_parameters[name];
        QList<QObject*> ch = G->children();
        foreach (QObject* obj, ch) {
...
{
    if (d->m_checkbox_parameters.contains(name))
        return d->m_checkbox_parameters[name]->setText(text);
    if (d->m_buttons.contains(name))
        return d->m_buttons[name]->setText(text);
}
