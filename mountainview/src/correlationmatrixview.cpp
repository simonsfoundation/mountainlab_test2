...
CorrelationMatrixView::CorrelationMatrixView(QWidget *parent) : QWidget(parent)
{
...
QColor CorrelationMatrixViewPrivate::get_color_at(QPointF pt)
{
	...
	int val0=val*256; if (val0<0) val0=0; if (val0>255) val0=255;
...
QPoint CorrelationMatrixViewPrivate::pixel2index(QPointF pt)
{
	...
	if ((xx<0)||(yy<0)) return QPoint(-1,-1);
	if ((xx>=N1)||(yy>=N2)) return QPoint(-1,-1);
...
void CorrelationMatrixViewPrivate::update_status()
{
	...
	if (m_hover_index.x()>=0) {
		...
		txt=QString("[%1,%2] : %3").arg(m_hover_index.x()+1).arg(m_hover_index.y()+1)
